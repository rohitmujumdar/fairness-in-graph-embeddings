...
df_train = df_positive_train #pd.concat([df_positive_train, df_negative_train])
df_test = pd.concat([df_positive_test, df_negative])

assert df_train.isnull().values.any() == False
assert df_test.isnull().values.any() == False
df_positive_test.to_csv('../data/df_test_20k.csv')
